...
const { db } = require('./db');
const { RECORDINGS_DIR, CHANNELS_CONF, TRANSCODE_MODE, TRANSCODE_CODEC } = require('./config');
...
const DVR = {
    init() {
        ...
        // Cleanup: Reset any "stuck" recordings from a previous session/crash
        db.run("UPDATE recordings SET status = 'failed' WHERE status = 'recording'", (err) => {
            if (err) logger.error('[DVR] Failed to cleanup stale recordings:', err);
            else logger.info('[DVR] Reset stale recording statuses.');
        });
    ...
    async checkTimers() {
        ...
        // 1. One-time timers
        db.all(
            "SELECT * FROM timers WHERE type = 'once' AND start_time <= ? AND end_time > ?",
            [now, now],
            (err, ones) => {
                if (err) return logger.error('[DVR] Error loading once timers:', err);
        ...
        db.all("SELECT * FROM timers WHERE type = 'series'", (err, series) => {
            if (err) return logger.error('[DVR] Error loading series timers:', err);
            series.forEach((timer) => {
                db.all(
                    'SELECT * FROM programs WHERE title = ? AND start_time <= ? AND end_time > ?',
                    [timer.title, now, now],
                    (err, progs) => {
                        if (err) return;
                        progs.forEach((p) => {
    ...
    async maybeStartRecording(timer, title, program = null) {
        ...
        db.get(
            'SELECT id FROM recordings WHERE channel_num = ? AND start_time = ?',
            [channelNum, startTime],
            async (err, row) => {
                if (row) return; // Already exists

                logger.info(`[DVR] Starting recording: ${title} on ${channel.name}...`);
                const tuner = await acquireTuner('dvr');
                if (!tuner) {
                    logger.warn(`[DVR] No tuners available for recording: ${title}`);
                    return;
                }

                const safeTitle = title.replace(/[^a-z0-9]/gi, '_').toLowerCase();
                let filename = `${safeTitle}_${startTime}.ts`;
                let filePath = path.resolve(RECORDINGS_DIR, filename);

                // Check for existing parts and handle resume scenario
                let partNum = 1;
                while (fs.existsSync(filePath)) {
                    partNum++;
                    filename = `${safeTitle}_${startTime}_part${partNum}.ts`;
                    filePath = path.resolve(RECORDINGS_DIR, filename);
                }

                if (partNum > 1) logger.info(`[DVR] Resuming recording as separate file: ${filename}`);

                // Insert into DB
                db.run(
                    `INSERT INTO recordings (title, description, channel_name, channel_num, start_time, end_time, file_path, status, timer_id) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                    [
                        title,
                    ...
                    function (err) {
                        if (err) {
                            ...
                            return logger.error('[DVR] Failed to log recording:', err);
                        }
                        const recId = this.lastID;
                        wsManager.broadcast('recording_status', { recId, title, status: 'starting', channelNum });
    ...
    async startProcess(recId, recKey, tuner, channel, filePath, endTime) {
        ...
        if (codec === 'copy') {
            ...
            if (engine === 'nvenc') {
                ...
                if (vEnc === 'h264_vaapi') {
                    // Fix "Access unit too large" error on some VA-API drivers by stripping SEI
        ...
        ffmpegArgs.push('-f', 'mpegts', filePath);

        zap.on('error', (err) => {
            logger.error(`[DVR] Failed to start Zap process: ${err.message}`);
            tuner.inUse = false;
            tuner.usageType = null;
        });
        ...
        zap.on('exit', (code) => {
            if (code !== 0 && code !== null) {
                logger.error(`[DVR] Zap process for Recording ${recId} exited with error code ${code}`);
            }
        });
        ...
        ffmpeg.on('error', (err) => {
            logger.error(`[DVR] Failed to start FFmpeg process: ${err.message}`);
            tuner.inUse = false;
            tuner.usageType = null;
            zap.kill('SIGKILL');
        });
        ...
        zap.stderr.on('data', (d) => {
            ...
            if (msg.includes('error') || msg.includes('fail')) logger.warn(`[DVR Debug] Zap Error: ${msg}`);
        });

        ffmpeg.stderr.on('data', (d) => {
            const msg = d.toString().trim();
            if (msg.includes('Error')) logger.error(`[DVR Debug] FFmpeg Stderr: ${msg}`);
            else logger.debug(`[DVR Debug] FFmpeg Stderr: ${msg}`);
        });
        ...
        ffmpeg.on('exit', (code) => {
            logger.info(`[DVR] FFmpeg for Recording ${recId} exited with code ${code}`);
            // code 0 = normal exit, null = SIGTERM (our stop), 255 = often SIGTERM-ish in some shells
            const status = code === 0 || code === null || code === 255 ? 'completed' : 'failed';
            db.run('UPDATE recordings SET status = ? WHERE id = ?', [status, recId]);
            wsManager.broadcast('recording_status', { recId, status, channelNum: channel.number });
            tuner.inUse = false;
            tuner.usageType = null;
    ...
    stopRecording(recKey) {
        ...
        if (!rec) return;
        logger.info(`[DVR] Stopping recording ${rec.recId}...`);
