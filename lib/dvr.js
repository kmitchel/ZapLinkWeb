...
const DVR = {
    init() {
        ...
        db.run("UPDATE recordings SET status = 'failed' WHERE status = 'recording'", (err) => {
            if (err) console.error('[DVR] Failed to cleanup stale recordings:', err);
            else console.log('[DVR] Reset stale recording statuses.');
        });
    ...
    async maybeStartRecording(timer, title, program = null) {
        ...
        db.get("SELECT id FROM recordings WHERE channel_num = ? AND start_time = ?", [channelNum, startTime], async (err, row) => {
            ...
            const filename = `${safeTitle}_${startTime}.ts`;
