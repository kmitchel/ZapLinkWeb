...
function getLatestLogos() {
    try {
        ...
        logger.warn('[Config] Failed to parse logos.json:', e);
    }
    return {};
}

async function downloadImages() {
    logger.info('[Icons] Starting icon download check...');
    try {
                ...
                https
                    .get(url, (response) => {
                        if (response.statusCode !== 200) {
                            logger.warn(`[Icons] Failed to download ${url}: Status ${response.statusCode}`);
                        ...
                        file.on('finish', () => {
                            file.close(() => {
                                LOCAL_ICON_MAP[key] = webPath;
                                logger.info(`[Icons] Cached: ${key} -> ${webPath}`);
                                resolve();
                            });
                        });
                    })
                    .on('error', (err) => {
                        logger.error(`[Icons] Error downloading ${url}:`, err.message);
                        fs.unlink(localPath, () => {});
                        resolve();
                    });
            });
        ...
        logger.info(`[Icons] Download check complete. ${Object.keys(LOCAL_ICON_MAP).length} icons locally cached.`);
    } catch (e) {
        logger.error('[Icons] Critical error during download:', e);
    }
}
...
function matchIcon(logos, channel) {
    ...
    // logger.debug(`matchIcon: checking for ${channelNum} / ${channelName} in ${Object.keys(logos).length} logos`);
    ...
    return (
        ...
        logos[channelNum] ||
        logos[channelName] ||
        Object.entries(logos).find(([k, _v]) => k.toLowerCase() === channelName.toLowerCase())?.[1] ||
        channel.icon ||
        null
    );
}
...
function loadChannels() {
    logger.info(`Loading channels from ${CHANNELS_CONF}...`);
    try {
        if (!fs.existsSync(CHANNELS_CONF)) {
            logger.warn('Channels config not found, using empty list.');
            return;
        }
        ...
        logger.info(`[Config] Loaded ${Object.keys(logos).length} icons from logos.json`);
        ...
        logger.info(`[Config] Matched ${iconCount} icons in channels.conf`);

        logger.info(`Loaded ${CHANNELS.length} channels:`);
        CHANNELS.forEach((c) =>
            logger.info(`  - ${c.name} (Service ID: ${c.serviceId}, Chan: ${c.number})${c.icon ? ' [ICON FOUND]' : ''}`)
        );

        if (iconCount > 0) {
            const sample = CHANNELS.find((c) => c.icon);
            logger.info(`[Config] Icon sample: ${sample.number} -> ${sample.icon}`);
        }
        ...
        logger.error('Failed to parse channels.conf:', e);
    }
}
