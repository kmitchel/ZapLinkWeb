...
const { db } = require('./db');
const { escapeXml, formatXmltvDate, debugLog } = require('./utils');
const { handleStream, handleFileStream } = require('./stream');
const { ENABLE_EPG } = require('./config');
const logger = require('./logger');

function setupRoutes(app, buildNumber) {
    // API: App Version
    app.get('/api/version', (req, res) => res.json({ version: buildNumber }));
    ...
    app.get('/playlist.m3u', (req, res) => {
        ...
        Channels.CHANNELS.forEach((channel) => {
            const icon = Channels.matchIcon(logos, channel);
    ...
    app.get('/xmltv.xml', (req, res) => {
        const sendXml = (rows = []) => {
            ...
            Channels.CHANNELS.forEach((c) => {
                const icon = Channels.matchIcon(logos, c);
                ...
                const channel = Channels.CHANNELS.find((c) => {
                    if (p.frequency) {
                        return (
                            ...
                            (c.number == p.channel_service_id || c.serviceId == p.channel_service_id)
                        );
                    }
        ...
        db.all(
            'SELECT * FROM programs WHERE end_time > ? ORDER BY frequency, channel_service_id, start_time',
            [Date.now()],
            (err, rows) => {
                if (err) {
                    logger.error(err);
                ...
                const program = rows.find((p) => {
                    if (p.frequency) {
                        return (
                            ...
                            (channel.number == p.channel_service_id || channel.serviceId == p.channel_service_id)
                        );
                    }
                ...
                return {
                    ...
                    now_playing: program
                        ? {
                            ...
                            progress: Math.round(
                                ((now - program.start_time) / (program.end_time - program.start_time)) * 100
                            )
                        }
                        : null
                };
    ...
    // API: Channel Specific Guide (next 7 days)
    app.get('/api/guide/channel/:channelNum', (req, res) => {
        if (!ENABLE_EPG) {
        ...
        const start = Date.now() - 6 * 60 * 60 * 1000; // 6 hours ago to show recently aired
        const end = Date.now() + 7 * 24 * 60 * 60 * 1000; // 7 days ahead

        db.all(
            'SELECT * FROM programs WHERE end_time > ? AND start_time < ? ORDER BY start_time',
            [start, end],
            (err, rows) => {
                if (err) {
                    logger.error(err);
                ...
                db.all('SELECT * FROM timers', (err, timers) => {
                    db.all("SELECT * FROM recordings WHERE status IN ('recording', 'failed')", (err, recs) => {
                        let programs = rows.filter((p) => {
                            const chanNum =
                                channel.naturalNumber ||
                                (channel.number ? channel.number.toString().replace('-', '.') : '');
                            const progNum = p.channel_service_id
                                ? p.channel_service_id.toString().replace('-', '.')
                                : '';
                        ...
                        const mappedPrograms = programs.map((p) => {
                            const recEntry =
                                ...
                                recs.find(
                                    (r) =>
                                        (r.channel_num == channel.number || r.channel_num == channel.naturalNumber) &&
                                        r.start_time === p.start_time
                                );

                            const isRecording = recEntry && recEntry.status === 'recording';
                            const isFailed = recEntry && recEntry.status === 'failed';
                            ...
                            const isScheduled =
                                ...
                                timers.some(
                                    (t) =>
                                        (t.type === 'once' &&
                                            (t.channel_num == channel.number ||
                                                t.channel_num == channel.naturalNumber) &&
                ...
                db.all('SELECT * FROM timers', (err, timers) => {
                    db.all("SELECT * FROM recordings WHERE status IN ('recording', 'failed')", (err, recs) => {
                        ...
                        const channels = Channels.CHANNELS.map((channel) => {
                            const icon = Channels.matchIcon(logos, channel);
                            logger.debug(
                                `[DEBUG] Guide generation - Channel: ${channel.number} (${channel.name}) - Icon Found: ${!!icon} (${icon ? 'Yes' : 'No'})`
                            );
                            ...
                            return {
                                ...
                                programs: programs.map((p) => {
                                    const recEntry =
                                        ...
                                        recs.find(
                                            (r) =>
                                                (r.channel_num == channel.number ||
                                                    r.channel_num == channel.naturalNumber) &&
                                                r.start_time === p.start_time
                                        );
                        ...
                        debugLog(`[API] Sending guide for ${channels.length} channels (${iconCount} with icons)`);
                        res.json({ channels });
                    });
                });
            }
    ...
    app.post('/api/timers', (req, res) => {
        ...
        db.run(
            'INSERT INTO timers (type, title, channel_num, start_time, end_time) VALUES (?, ?, ?, ?, ?)',
    ...
    app.delete('/api/recordings/:id', (req, res) => {
        db.get('SELECT file_path FROM recordings WHERE id = ?', [req.params.id], (err, row) => {
            if (row) {
                logger.info(`[DVR] Deleting recording file: ${row.file_path}`);
                if (fs.existsSync(row.file_path)) {
                    try {
                        fs.unlinkSync(row.file_path);
                        logger.info(`[DVR] Successfully deleted: ${row.file_path}`);
                    } catch (e) {
                        logger.error(`[DVR] Failed to delete file: ${row.file_path}`, e);
                    }
                } else {
                    logger.warn(`[DVR] File not found on disk: ${row.file_path}`);
                }
            }
