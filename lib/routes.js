            ...
            db.all("SELECT * FROM timers", (err, timers) => {
                db.all("SELECT * FROM recordings WHERE status IN ('recording', 'failed')", (err, recs) => {
                    ...
                    const channels = Channels.CHANNELS.map(channel => {
                        const icon = Channels.matchIcon(logos, channel);
                        console.log(`[DEBUG] Guide generation - Channel: ${channel.number} (${channel.name}) - Icon Found: ${!!icon} (${icon ? 'Yes' : 'No'})`);
                        ...
                        return {
                            ...
                            programs: programs.map(p => {
                                const recEntry = recs && recs.find(r =>
                                ...
                                const isScheduled = !isRecording && !isFailed && timers && timers.some(t =>
                                    (t.type === 'once' && (t.channel_num == channel.number || t.channel_num == channel.naturalNumber) && t.start_time === p.start_time) ||
