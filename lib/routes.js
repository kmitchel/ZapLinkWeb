...
function setupRoutes(app, buildNumber) {
    ...
    app.get('/playlist.m3u', (req, res) => {
        ...
        Channels.CHANNELS.forEach(channel => {
            const hyphenated = channel.number.toString().replace('.', '-');
            const natural = channel.naturalNumber || channel.number.toString().replace('-', '.');
            const icon = logos[natural] || logos[hyphenated] || logos[channel.number] || logos[channel.name] || channel.icon;
    ...
    app.get('/xmltv.xml', (req, res) => {
        const sendXml = (rows = []) => {
            ...
            Channels.CHANNELS.forEach(c => {
                const hyphenated = c.number.toString().replace('.', '-');
                const natural = c.naturalNumber || c.number.toString().replace('-', '.');
                const icon = logos[natural] || logos[hyphenated] || logos[c.number] || logos[c.name] || c.icon;
