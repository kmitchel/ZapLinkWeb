...
const { CHANNELS_CONF, ENABLE_TRANSCODING, ENABLE_QSV, ENABLE_NVENC, VERBOSE_LOGGING } = require('./config');
...
async function handleStream(req, res) {
    ...
    const ffmpegArgs = [];
    if (ENABLE_NVENC) {
        // NVENC usually doesn't need explicit hw device init for this setup
    } else if (ENABLE_QSV) {
    ...
    if (ENABLE_NVENC) {
    ...
    } else if (ENABLE_TRANSCODING) {
        // Priority 3: Software (libx264)
        ffmpegArgs.push(
            '-c:v', 'libx264',
            '-preset', 'ultrafast',
            '-tune', 'zerolatency',
        ...
        // Priority 4: Direct Stream (Pass-through)
    ...
    zap.on('exit', (code, signal) => {
        ...
        // If zap exits naturally or via SIGTERM from cleanup
        ...
        }, 500); // Small grace period for OS to release device handles
    });
