...
const EPG = {
    ...
    async grab(scanTimeout = 15000) {
        ...
        if (freeTuners.length === 0) {
            logger.warn('[EPG] No tuners available, skipping scan.');
        ...
        try {
            logger.info(`[EPG] Starting parallel scan on ${freeTuners.length} tuners...`);
            ...
            let processedCount = 0; // Initialize processedCount here

            const runScanner = async (tuner) => {
                while (queue.length > 0) {
                    ...
                    wsManager.broadcast('tuner_status', { tunerId: tuner.id, status: 'scanning' }); // Broadcast when tuner starts scanning
                    const channelName = muxMap.get(freq);

                    // Increment processed count before broadcasting
                    processedCount++;
                    wsManager.broadcast('epg_scan_progress', {
                        status: 'scanning',
                        frequency: freq,
                        channel: channelName,
                        progress: Math.round((processedCount / frequencies.length) * 100)
                    });

                    logger.info(`[EPG] Tuner ${tuner.id} -> ${freq} Hz (${channelName})`);
                    try {
            ...
            // Launch workers for each free tuner
            await Promise.all(freeTuners.map((t) => runScanner(t)));
        } catch (e) {
            logger.error('[EPG] Critical error during grab:', e);
        } finally {
            this.isScanning = false;
            this.lastScan = Date.now();
            this.isInitialScanDone = true;
            logger.info('[EPG] All parallel scans complete.');
        }
    },
    ...
    scanMux(tuner, channelName, freq, scanTimeout) {
        return new Promise((resolve) => {
            const args = [
                ...
                '-t',
                Math.ceil(scanTimeout / 1000).toString(),
                '-o',
                '-',
                channelName
            ];
            ...
            zap.stdout.on('data', (data) => {
                if (!dataReceived) {
                    logger.info(`[EPG] Receiving data stream for ${channelName}...`);
                    dataReceived = true;
                }
                buffer = Buffer.concat([buffer, data]);
                if (buffer.length > 50 * 1024 * 1024) {
                    // 50MB limit for full mux scan
                    zap.kill('SIGKILL');
                }
            });

            zap.stderr.on('data', (d) => {
                logger.debug(`[EPG Debug] Zap stderr: ${d.toString()}`);
            });

            const timeout = setTimeout(() => {
                if (!dataReceived)
                    logger.warn(
                        `[EPG] No data received for ${channelName} after ${scanTimeout / 1000}s. Signal might be weak.`
                    );
                zap.kill('SIGKILL');
            ...
            zap.on('exit', () => {
                ...
                logger.info(`[EPG] Mux scan finished. Discovered ${count} program entries.`);
                resolve();
            });
        });
    },
    ...
    parseEIT(buffer, freq) {
        ...
        logger.debug(`[EPG] Beginning parse of ${buffer.length} bytes for frequency ${freq}...`);
            ...
            if (pusi) {
                ...
                if (state) {
                    ...
                    if (state.buffer.length >= state.totalLength) {
                        this.handleCompleteSection(
                            state.buffer.slice(0, state.totalLength),
                            pid,
                            stats,
                            tableCounts,
                            sections,
        ...
        const sortedPids = Array.from(pidCounts.entries())
            .sort((a, b) => b[1] - a[1])
            .slice(0, 10);
        logger.debug('[EPG] Top 10 PIDs found:', Object.fromEntries(sortedPids));
        logger.debug(
            `[EPG] Guide PIDs seen? DVB(18): ${pidCounts.get(18) || 0}, ATSC(8187): ${pidCounts.get(8187) || 0}`
        );

        const tableSummary = {};
        tableCounts.forEach((v, k) => (tableSummary[`0x${k.toString(16).toUpperCase()}`] = v));
        logger.debug('[EPG] ATSC Tables found:', tableSummary);
        ...
        try {
            ...
            for (let i = 0; i < numChannels; i++) {
                ...
                if (sourceId) {
                    ...
                    if (!channel) {
                        channel = Channels.CHANNELS.find(
                            (c) => c.frequency == freq && c.serviceId == programNumber.toString()
                        );
                    }
                    if (!channel) {
                    ...
                    if (channel) {
                        if (this.sourceMap.get(mapKey) !== channel.number) {
                            logger.debug(
                                `[ATSC VCT] Map: ${freq} Source ${sourceId} -> ${channel.name} (${channel.number})`
                            );
                            this.sourceMap.set(mapKey, channel.number);
                        }
                    } else {
                        if (!this.sourceMap.has(mapKey)) {
                            logger.debug(`[ATSC VCT] Unconfigured: ${freq} Source ${sourceId} -> ${virtualChannel}`);
    ...
    parseATSCEIT(section, virtualChannel, onFound, sourceId, freq) {
        try {
            ...
            for (let i = 0; i < numEvents; i++) {
                ...
                const duration =
                    ((section[offset + 6] & 0x0f) << 16) | (section[offset + 7] << 8) | section[offset + 8];
                    ...
                    if (titleBuffer.length > 0) {
                        ...
                        if (numStrings > 0 && titleBuffer.length >= stringOffset + 7) {
                            ...
                            if (titleBuffer.length >= stringOffset + 7 + stringLen) {
                                title = titleBuffer
                                    .slice(stringOffset + 7, stringOffset + 7 + stringLen)
                                    .toString('utf8');
                ...
                if (currentEventOffset + 2 <= section.length - 4) {
                    const descriptorsLength =
                        ((section[currentEventOffset] & 0x0f) << 8) | section[currentEventOffset + 1];
            ...
            logger.error('[ATSC DEBUG] Error parsing EIT:', e);
        }
    },
    ...
    parseATSCEET(section, sourceId, virtualChannel, freq) {
        try {
            ...
            if (desc) {
                logger.debug(`[ATSC ETT] Decoded Desc for Chan ${virtualChannel} Event ${eventId}`);
                db.run(
                    'UPDATE programs SET description = ? WHERE frequency = ? AND channel_service_id = ? AND event_id = ?',
                    [desc, freq, virtualChannel, eventId]
                );
            }
        } catch (e) {
            logger.error('[ATSC ETT] Error:', e);
        }
    },
    ...
    parseDVBEIT(section, serviceId, onFound, freq) {
        try {
            ...
            while (evOffset < sectionLength - 1) {
                ...
                const startTimeBCD =
                    (section[evOffset + 4] << 16) | (section[evOffset + 5] << 8) | section[evOffset + 6];
                const durationBCD =
                    (section[evOffset + 7] << 16) | (section[evOffset + 8] << 8) | section[evOffset + 9];
                ...
                const durationSec =
                    ((durationBCD >> 16) & 0x0f) * 3600 +
                    ((durationBCD >> 20) & 0x0f) * 36000 +
                    ((durationBCD >> 8) & 0x0f) * 60 +
                    ((durationBCD >> 12) & 0x0f) * 600 +
                    (durationBCD & 0x0f) +
                    ((durationBCD >> 4) & 0x0f) * 10;
                ...
                while (descOffset < evOffset + 12 + descriptorsLength) {
                    ...
                    if (tag === 0x4d) {
                        ...
                        if (section[titleStart] < 0x20) {
                            titleStart++;
                            titleLen--;
                        }
                        title = section
                            .slice(titleStart, titleStart + titleLen)
                            .toString('utf8')
                            .replace(/[^\x20-\x7E]/g, '');
                    } else if (tag === 0x4e) {
                        // Extended Event
                        if (!desc) {
                            ...
                            if (textOffset + 3 < descOffset + 2 + len) {
                                ...
                                if (remainingLen > 0) {
                                    desc = section
                                        .slice(textOffset, textOffset + remainingLen)
                                        .toString('utf8')
                                        .trim();
                                }
                            }
                ...
                if (title && startTime > 0) {
                    onFound();
                    db.run(
                        `INSERT INTO programs (frequency, channel_service_id, start_time, end_time, title, description) 
                            VALUES (?, ?, ?, ?, ?, ?)
                            ON CONFLICT(frequency, channel_service_id, start_time) 
