...
typedef enum {
    LOG_LVL_ERROR,  /**< Critical errors that may cause failure */
    LOG_LVL_WARN,   /**< Warning conditions */
    LOG_LVL_INFO,   /**< Informational messages */
    LOG_LVL_DEBUG   /**< Verbose debug output (requires -v flag) */
} LogLevel;
...
/**
 * Format current time as HH:MM:SS into provided buffer
 */
...
#define LOG(level, tag, fmt, ...) do { \
    if ((level) == LOG_LVL_DEBUG && !g_verbose) break; \
    char _ts[16]; log_timestamp(_ts, sizeof(_ts)); \
    const char *_color = ""; \
    const char *_prefix = ""; \
    switch (level) { \
        case LOG_LVL_ERROR: _color = COLOR_RED; _prefix = "ERROR"; break; \
        case LOG_LVL_WARN:  _color = COLOR_YELLOW; _prefix = "WARN "; break; \
        case LOG_LVL_INFO:  _color = COLOR_GREEN; _prefix = "INFO "; break; \
        case LOG_LVL_DEBUG: _color = COLOR_DIM; _prefix = "DEBUG"; break; \
    } \
...
#define LOG_ERROR(tag, fmt, ...) LOG(LOG_LVL_ERROR, tag, fmt, ##__VA_ARGS__)

/** Log a warning message */
#define LOG_WARN(tag, fmt, ...)  LOG(LOG_LVL_WARN, tag, fmt, ##__VA_ARGS__)

/** Log an informational message */
#define LOG_INFO(tag, fmt, ...)  LOG(LOG_LVL_INFO, tag, fmt, ##__VA_ARGS__)

/** Log a debug message (only visible with -v flag) */
#define LOG_DEBUG(tag, fmt, ...) LOG(LOG_LVL_DEBUG, tag, fmt, ##__VA_ARGS__)
